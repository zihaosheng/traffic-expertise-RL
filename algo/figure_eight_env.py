    ...
    * ring_length: bounds on the ranges of ring road lengths the autonomous
    ...
    def reset(self):
        ...
        radius_ring = random.randint(
            self.env_params.additional_params['radius_ring'][0],
            self.env_params.additional_params['radius_ring'][1])
        ...
        print('\n-----------------------')
        print('radius ring:', net_params.additional_params['radius_ring'])
        ...
        self.rl_id = self.k.vehicle.get_rl_ids()[0]
        self.follow_id = self.k.vehicle.get_follower(self.rl_id)
        self.lead_id = self.k.vehicle.get_leader(self.rl_id)
        return observation


class Figure8POEnv(WaveAttenuationEnv):
    ...
    * ring_length: bounds on the ranges of ring road lengths the autonomous
    ...
    def get_state(self):
        ...
        if self.env_params.additional_params['radius_ring'] is not None:
            max_length = self.env_params.additional_params['radius_ring'][1]*4*np.pi
        else:
            max_length = self.k.network.length()

        # state = [rl speed, speed difference, distance from RL to follow vehicle]
    ...
    def compute_reward0(self, rl_actions, **kwargs):
        """See class definition."""
